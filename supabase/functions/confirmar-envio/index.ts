// Arquivo: supabase/functions/confirmar-envio/index.ts (VERSÃO FINAL COM SELEÇÃO DE LOGO)
import { createClient } from 'https://esm.sh/@supabase/supabase-js@2'
import { corsHeaders } from '../_shared/cors.ts'

// Funções Auxiliares
function gerarHtmlEtiqueta(envio: any): string {
    // --- LÓGICA DE SELEÇÃO DO LOGO DA EMPRESA ---
    ...
    // Converte a marca para minúsculas para comparação segura
    ...
    for (let i = 1; i <= envio.volumes; i++) {
        etiquetasHtml += `
            <div class="label">
                <div class="header">
                    <img src="${logoDaEmpresa}" class="company-logo">
                    ${envio.carrier_logo ? `<img src="${envio.carrier_logo}" class="carrier-logo">` : ''}
                </div>
    ...
    // O estilo HTML/CSS é mantido inalterado do primeiro código
...
// Função Principal Deno.serve (mantida inalterada no corpo)
Deno.serve(async (req) => {
    ...
    try {
        ...
        // 1. Atualiza a venda no banco de dados (o campo 'marca' deve estar sendo retornado aqui)
        const { data: vendaAtualizada, error } = await supabase
            .from('envios')
            .update({ volumes: volumes, status: 'Confirmado' })
            .eq('id', envio_id)
            .select() // Importante: Garante que os dados (incluindo 'marca') são retornados
            .single();
        ...
        // 3. Gera o HTML da etiqueta com os dados atualizados e o logo correto
