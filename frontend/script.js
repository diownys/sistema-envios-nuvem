<!-- Carregue como módulo no HTML -->
<script type="module">
// --- CONFIGURAÇÕES GERAIS ---
import { createClient } from 'https://esm.sh/@supabase/supabase-js'

// Planilhas públicas (CSV)
...
// API/DB
const API_URL_STATS = 'http://100.97.126.124:8000/api/dashboard-stats'; // (não usado aqui, mas mantido)
...
let coletasSchedule = []; // cache de coletas para não buscar a cada segundo

// --- UTIL: CSV ---
async function fetchAndParseCsv(url) {
  const response = await fetch(url + '&cachebust=' + Date.now());
  if (!response.ok) throw new Error(`Falha ao buscar CSV: ${url}`);
  const csvText = (await response.text()).trim().replace(/\r/g, '');
  ...
  return lines.slice(1).map(line => {
    ...
    headers.forEach((h, i) => {
      ...
      obj[h] = raw.replace(/^"(.*)"$/, '$1'); // remove aspas externas
    });
    return obj;
  });
}
...
async function updateApiData() {
  try {
    ...
    console.log('🔍 Total de registros do Supabase:', envios?.length);
    console.log('Primeiro registro:', JSON.stringify(envios?.[0], null, 2));

    // 🔹 Filtra envios do dia
    const hojeISO = new Date().toISOString().slice(0, 10);
    const enviosHoje = (envios || []).filter(e => {
      const dt = e?.created_at ? new Date(e.created_at) : null;
      if (!dt || isNaN(dt)) return false;
      return dt.toISOString().slice(0, 10) === hojeISO;
    });
    ...
    // 🔹 Valor expedido (somente concluídos)
    const valorExpedido = concluidosHoje.reduce((acc, e) => {
      const val = Number(e.valor_total ?? e.valor_venda ?? 0);
    ...
    // 🔹 Envios concluídos por UF (mapa)
    const enviosPorUF = {};
    for (const e of concluidosHoje) {
      const uf = String((e.uf || e.estado || '')).trim().toUpperCase().slice(0, 2);
      if (!uf) continue;
      enviosPorUF[uf] = (enviosPorUF[uf] || 0) + 1;
    }

    // === Atualiza UI ===
    const totalEl = document.getElementById('total-envios');
    if (totalEl) totalEl.textContent = totalConcluidosHoje.toString();

    const valorEl = document.getElementById('valor-total');
    if (valorEl) valorEl.textContent = valorExpedido.toLocaleString('pt-BR', { minimumFractionDigits: 2 });

    // ⚠️ Ordem corrigida: (pendentes, concluidos)
    updateProgressChart(totalPendentesHoje, totalConcluidosHoje);
    ...
    console.log('Envios concluídos por UF:', enviosPorUF);

  } catch (err) {
    console.error('Erro ao buscar dados do Supabase:', err);
    const safe = (id, text = '---') => { const el = document.getElementById(id); if (el) el.textContent = text; };
    safe('total-envios'); safe('valor-total');

    const mapC = document.getElementById('map-container');
    if (mapC) mapC.innerHTML = `<p style="color:#ff6b6b;text-align:center;">Erro ao buscar dados</p>`;

    const janC = document.getElementById('janela-stats-blocks');
    if (janC) janC.innerHTML = `<p style="color:#ff6b6b;text-align:center;">Erro ao buscar dados</p>`;
  }
}
...
async function fetchColetasSchedule() {
  try {
    if (!URL_COLETAS) throw new Error("URL_COLETAS não foi definida no script.js.");
...
async function updateCountdowns() {
  try {
    ...
    todaySchedule.forEach(item => {
      const [h, m] = String(item.time || '00:00').split(':');
      const targetTime = new Date();
      targetTime.setHours(Number(h||0), Number(m||0), 0, 0);
      const diff = targetTime - now;
      const obj = { ...item, targetTime, diff };
      (diff > 0 ? pendentes : finalizadas).push(obj);
    });

    pendentes.sort((a, b) => a.targetTime - b.targetTime);
    finalizadas.sort((a, b) => b.targetTime - a.targetTime);

    const processed = [...pendentes, ...finalizadas];
    ...
    processed.forEach(item => {
      ...
      itemDiv.innerHTML = `
        <div class="info">
          ...
          <span class="time">Limite: ${item.time || '--:--'}</span>
        </div>
...
function getWeather() {
  ...
  fetch(`https://api.openweathermap.org/data/2.5/weather?q=${city},BR&appid=${apiKey}&lang=pt_br&units=metric`)
    .then(r => r.ok ? r.json() : Promise.reject('Erro de Clima'))
    .then(data => {
      const c = id => document.getElementById(id);
      c('city-name') && (c('city-name').textContent = data.name);
      c('temperature') && (c('temperature').textContent = `${Math.round(data.main.temp)}°C`);
      c('description') && (c('description').textContent = data.weather[0].description);
      c('weather-icon') && (c('weather-icon').src = `https://openweathermap.org/img/wn/${data.weather[0].icon}@2x.png`);
...
function createOrUpdateBarChart(chart, canvasId, data, label, color, hoverColor) {
  ...
  return new Chart(ctx, {
    type: 'bar',
    data: { labels, datasets: [{ label, data: values, backgroundColor: color, hoverBackgroundColor: hoverColor, borderWidth: 1, borderRadius: 4 }] },
    options: {
...
function getOccurrences() {
  fetch('https://atlas-sa-ocorrencias.netlify.app/.netlify/functions/get-top-occurrences')
    .then(r => r.json())
    .then(data => {
      if (data.internas?.length) {
        internasChart = createOrUpdateBarChart(internasChart, 'internas-chart',
          data.internas.slice(0, 3), 'Ocorrências Internas', 'rgba(240, 196, 76, 0.7)', '#f0c44c');
      }
      if (data.externas?.length) {
        externasChart = createOrUpdateBarChart(externasChart, 'externas-chart',
          data.externas.slice(0, 3), 'Ocorrências Externas', 'rgba(76, 175, 80, 0.7)', '#4caf50');
      }
    }).catch(console.error);
}
...
async function getBirthdays() {
  try {
    const all = await fetchAndParseCsv(URL_ANIVERSARIANTES);
    const today = new Date();
    const currentMonth = today.getMonth() + 1;
    const currentDay   = today.getDate();
    const monthBirthdays = all.filter(p => Number(p.month) === currentMonth)
  ...
  } catch (e) { console.error('Erro ao carregar aniversariantes:', e); }
}
      ...
      data.forEach(item => {
        list.innerHTML += `
          <div class="recognition-item">
            <div class="header"><span>${item.author}</span> reconhece: <span>${item.recognized}</span></div>
            <p class="message">"${item.message}"</p>
          </div>`;
      });
    }
  } catch (e) { console.error('Erro ao carregar reconhecimentos:', e); }
}
...
async function getNews() {
  try {
    ...
    if (data?.length && data[0].message) {
      const newsString = data.map(i => i.message).filter(Boolean).join(' • ');
...
function updateJanelaBlocks(janelas) {
  ...
  janelas.sort((a,b) => String(a.janela_coleta).localeCompare(String(b.janela_coleta)));
  const maxPend = Math.max(...janelas.map(i => i.total), 0);
  janelas.forEach(item => {
    let fontSize = 1.4;
    if (maxPend > 0) fontSize = 1.2 + ((item.total / maxPend) * 1.8);
    container.innerHTML += `
      <div class="janela-block">
        <div class="janela-block-name">${item.janela_coleta}</div>
        <div class="janela-block-value" style="font-size:${fontSize.toFixed(1)}em;">${item.total}</div>
      </div>`;
  });
}
...
function updateProgressChart(pendentes, concluidos) {
  ...
  if (txt) {
    txt.innerHTML = `<div id="progress-percent">${percentual}%</div><div id="progress-details">${concluidos} de ${total}</div>`;
  }
  if (progressChart) {
...
async function updateMap(destinos) {
  try {
    ...
    const height = container.clientHeight || width * 0.9;
    ...
    if (c) c.innerHTML = `<p style="color: #ff6b6b; text-align:center;">Erro: ${e.message}</p>`;
  }
}
...
function setupIntervals() {
  ...
  setInterval(updateCountdowns, 1000);   // contagem regressiva (sem rebuscar CSV)
  setInterval(updateApiData, 300000);    // dados do dashboard a cada 5 minutos
}

document.addEventListener('DOMContentLoaded', () => {
  // Data do cabeçalho compacto (opcional)
  const dateTimeElement = document.getElementById('date-time');
  if (dateTimeElement) {
    dateTimeElement.textContent = new Date().toLocaleDateString('pt-BR', { weekday: 'long', year:'numeric', month:'long', day:'numeric' });
  }
  // Carrosséis (se existirem)
...
function startDashboard() {
  ...
  updateClock();
  // dispara 1x para popular tudo rapidamente
  updateApiData();
}

// -------- MOTION KIT v2 (preservado) --------
// (sem alterações funcionais — apenas mantido igual)
(function () {
  const TARGETS = [
  ...
  function formatValue(value, mode) {
    if (mode === 'currency') return Number(value).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
...
(function () {
  ...
  function getUFAndValue(target) {
    const datum = d3.select(target).datum(); // d3 data bound
  ...
  function onEnter(e) { const { uf, val } = getUFAndValue(e.currentTarget); if (!uf) return; show(e.clientX, e.clientY, uf, val); }
  function onMove(e)  { const { uf } = getUFAndValue(e.currentTarget); if (!uf) return; tip.style.left = `${e.clientX}px`; tip.style.top = `${e.clientY}px`; }
...
(function () {
  ...
  function fireConfetti(ms = 1600) {
    ...
    Object.assign(cvs.style, { position:'fixed', inset:'0', width:'100vw', height:'100vh', pointerEvents:'none', zIndex:9998 });
    ...
    function draw(now) {
      ...
      for (const p of parts) {
        p.x += p.vx; p.y += p.vy;
        ctx.save(); ctx.translate(p.x, p.y); ctx.rotate(now * p.vr / 1000);
    ...
    const onResize = () => { stop = true; cvs.remove(); removeEventListener('resize', onResize); };
    addEventListener('resize', onResize);
  }
})();

// Plugin Chart.js (glow)
(function () {
...
// --- Bootstrap ---
window.addEventListener('load', () => {
  startDashboard();     // prepara UI e busca tudo 1x
  setupIntervals();     // timers contínuos
});
</script>
